...
* **Scroll cues are banned.** `Scroll`, `down scroll`, `Scroll to explore`, `Scroll to walk through it`, animated mouse-wheel icons. If the user has not scrolled yet, they are looking at the hero. They know what scroll is. The bottom of the viewport does not need a label.
...
If your output contains a single em-dash or en-dash-as-separator anywhere visible to the user, the output fails the Pre-Flight Check and must be rewritten.
...
The Reference Vocabulary (Section 10) names patterns. The Block Library implements them.
...
- [ ] **Page Theme Lock**: ONE theme for the whole page. No section flips mid-page.
...
- [ ] **Serif discipline**: NOT Fraunces or Instrument_Serif unless explicitly justified.
- [ ] **Premium-consumer palette check**: NOT the beige+brass+espresso family by default.
...
- [ ] **"Trusted by" logo wall UNDER the hero**, uses real SVG logos.
- [ ] **Copy Self-Audit**: no broken grammar, no AI hallucination phrases.
...
- [ ] **Real images used** (no div-based fake screenshots, no decorative hand-rolled SVGs).
- [ ] **No pills/labels overlaid on images**.
- [ ] **No photo-credit captions as decoration**.
- [ ] **No version footers** on marketing pages.
- [ ] **No locale / city-name / time / weather strips** unless brief demands it.
- [ ] **No scroll cues**.
...
- [ ] **GSAP patterns** implemented per Section 5.A/5.B canonical skeletons.
...
- [ ] **Core Web Vitals** targets plausible (LCP < 2.5s, INP < 200ms, CLS < 0.1).
...
- Polaris: https://polaris-react.shopify.com/components
...
- Apple Liquid Glass (Apple platforms only): https://developer.apple.com/design/human-interface-guidelines/materials

## Appendix C - Apple Liquid Glass Web Approximation

No official `liquid-glass.css` from Apple exists for web. Label web implementations as approximations.
